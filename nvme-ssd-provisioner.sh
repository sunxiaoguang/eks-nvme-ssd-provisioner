...
if [[ -z $1 ]]
then
    echo "Missing required Instance Storage type argument"
fi

INSTANCE_STORAGE_TYPE=$1

set -o errexit
set -o nounset
set -o pipefail

mapfile -t SSD_NVME_DEVICE_LIST < <(nvme list | grep "$INSTANCE_STORAGE_TYPE" | cut -d " " -f 1 || true)
SSD_NVME_DEVICE_COUNT=${#SSD_NVME_DEVICE_LIST[@]}
if [[ $SSD_NVME_DEVICE_COUNT -eq "0" ]]
then
# Look for disks other than the system disk
mapfile -t SSD_NVME_DEVICE_LIST < <(ls /sys/block | grep -v vda | xargs -I. echo /dev/.|| true)
SSD_NVME_DEVICE_COUNT=${#SSD_NVME_DEVICE_LIST[@]}
fi
...
case $SSD_NVME_DEVICE_COUNT in
"0")
  echo "No devices found of type '$INSTANCE_STORAGE_TYPE'"
